...
const UD_FEATS={"PronType":["Art","Dem","Emp","Exc","Ind","Int","Neg","Prs","Rcp","Rel","Tot"],"NumType":["Card","Dist","Frac","Mult","Ord","Range","Sets"],"Poss":["Yes"],"Gender":["Com","Fem","Masc","Neut"],"Animacy":["Anim","Hum","Inan","Nhum"],"NounClass":["Bantu1","Bantu2","Bantu3","Bantu4","Bantu5","Bantu6","Bantu7","Bantu8","Bantu9","Bantu10","Bantu11","Bantu12","Bantu13","Bantu14","Bantu15","Bantu16","Bantu17","Bantu18","Bantu19","Bantu20","Bantu21","Bantu22","Bantu23","Wol1","Wol2","Wol3","Wol4","Wol5","Wol6","Wol7","Wol8","Wol9","Wol10","Wol11","Wol12"],"Number":["Coll","Count","Dual","Grpa","Grpl","Inv","Pauc","Plur","Ptan","Sing","Tri"],"Case":["Abs","Acc","Erg","Nom","Abe","Ben","Cau","Cmp","Cns","Com","Dat","Dis","Equ","Gen","Ins","Par","Tem","Tra","Voc","Abl","Add","Ade","All","Del","Ela","Ess","Ill","Ine","Lat","Loc","Per","Sbe","Sbl","Spl","Sub","Sup","Ter"],"Definite":["Com","Cons","Def","Ind","Spec"],"VerbForm":["Conv","Fin","Gdv","Ger","Inf","Part","Sup","Vnoun"],"Mood":["Adm","Cnd","Des","Imp","Ind","Int","Irr","Jus","Nec","Opt","Pot","Prp","Qot","Sub"],"Tense":["Fut","Imp","Past","Pqp","Pres"],"Aspect":["Hab","Imp","Iter","Perf","Prog","Prosp"],"Voice":["Act","Antip","Bfoc","Cau","Dir","Inv","Lfoc","Mid","Pass","Rcp"],"Reflex":["Yes"],"Abbr":["Yes"],"Evident":["Fh","Nfh"],"Typo":["Yes"],"Deixis":["Abv","Bel","Even","Med","Nvis","Prox","Remt"],"Polarity":["Neg","Pos"],"Foreign":["Yes"],"DeixisRef":["1","2"],"Person":["0","1","2","3","4"],"ExtPos":["ADJ","ADP","ADV","AUX","CCONJ","DET","INTJ","PRON","PROPN","SCONJ"],"Degree":["Abs","Aug","Cmp","Dim","Equ","Pos","Sup"],"Polite":["Elev","Form","Humb","Infm"],"Clusivity":["Ex","In"],"Shared":["Yes","No"],"Subject":["SubjRaising","ObjRaising","OblRaising","Generic","Instantiated","NoRaising"]};
...
const UD_MISC_KEYS=["SpaceAfter","SpacesAfter","SpacesBefore","Translit","LTranslit","Gloss","MSeg","MGloss","Lang","CorrectForm","CorrectSpaceAfter","Entity","NER","Cxn","Reported","ToDo"];   // item 7: Reported joins the offered keys so the MISC cell autocompletes it like any other
const UD_MISC_VALS={SpaceAfter:["No"],CorrectSpaceAfter:["Yes","No"],Reported:["Yes"]};   // most MISC keys are free-text → doc-mined values carry the rest
...
function buildFeatEditor(td,sent,t,si,i,key){
  ...
  const editPill=pill=>{ arm(); const cur=pill.dataset.val;   // turn the pill back into editable Key=Value text
    ...
    const openIeAC=force=>{ if(document.activeElement!==ie){ if(_acInput===ie)acCloseSoon(); return; }
      ...
      else { const descFn=(key==="feats"&&keyName&&UD_FEATS[keyName])?(v=>(FEATS_VDESC[keyName]||{})[v]||""):null; acShowCustom(ie,ms,pick,descFn); } };   // official value glosses, dimmed & right-aligned (FEATS only)
  ...
  const openPillAC=force=>{ if(document.activeElement!==box||box.querySelector("input.pilledit")){ if(_acInput===box)acCloseSoon(); return; }   // not the free-typing caret (e.g. a live chip-edit input) → nothing to complete
    ...
    else { const descFn=(key==="feats"&&keyName&&UD_FEATS[keyName])?(v=>(FEATS_VDESC[keyName]||{})[v]||""):null;   // item 12 (opt): each official value's gloss, dimmed & right-aligned. MISC & value lists otherwise stay flat
