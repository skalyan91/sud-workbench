...
/* ── WHICH WORD CLASSES A FEATURE BELONGS TO, AND WHICH OF ITS VALUES ────────────────────────────
   Both tables are DERIVED, not written by hand: from the UD validator's own permitted-features data
   (`data/feats.json` in UniversalDependencies/tools — what `validate.py` checks a treebank against),
   over its 286 languages, counting only features it marks `type:"universal"` and `permitted:1`.
   ⚠ A UNION OVER 286 LANGUAGES IS USELESS ON ITS OWN — some language permits VerbForm on PUNCT — so
   both tables apply ONE stated rule: keep a class (or a class+value pair) if at least 3 languages
   permit it AND it reaches 20 % of the count of whichever class permits that feature (or value) most.
   Recompute, don't edit by hand; the derivation is one pass over that file and the rule is the only
   parameter in it.
   20 % is where the class lists stop being a list of everything: at 10 % a NOUN still keeps 27 of the
   28 features (Tense, Aspect and Voice among them) and the grid's dropdown narrows to nothing worth
   having; at 20 % it keeps 21, dropping those three while keeping VerbForm, which is what a verbal
   noun needs. The feature-level table also corroborates the hand-curated UPOS_SUBTYPE_ON in
   js/editing/context-menu.js (VerbForm → VERB, AUX, ADJ, NOUN exactly), which is why that one is left
   as the gate for the subtype flyout and this only narrows within it — and FEAT_UPOS is UNIONED with
   it besides, so the two pickers for one FEATS column can never disagree about what a class may take.
   A feature ABSENT from FEAT_UPOS is unrestricted — either the data ties it to no class in particular
   (Abbr, Typo, Foreign apply to any word) or it is not the validator's to answer (ExtPos, Shared,
   Deixis, DeixisRef). Absent is therefore "no opinion", never "no classes". */
const FEAT_UPOS={"PronType":["ADJ","ADV","DET","PRON"],"NumType":["ADJ","ADV","DET","NUM"],"Poss":["ADJ","DET","NOUN","PRON"],"Gender":["ADJ","ADP","ADV","AUX","DET","NOUN","NUM","PRON","PROPN","VERB"],"Animacy":["ADJ","AUX","DET","NOUN","NUM","PRON","PROPN","VERB"],"NounClass":["ADJ","ADP","NOUN","PRON"],"Number":["ADJ","ADP","ADV","AUX","DET","NOUN","NUM","PART","PRON","PROPN","VERB"],"Case":["ADJ","ADP","ADV","AUX","DET","NOUN","NUM","PART","PRON","PROPN","VERB"],"Definite":["ADJ","ADV","DET","NOUN","NUM","PRON","PROPN","VERB"],"VerbForm":["ADJ","AUX","NOUN","VERB"],"Mood":["AUX","VERB"],"Tense":["AUX","VERB"],"Aspect":["AUX","VERB"],"Voice":["AUX","VERB"],"Reflex":["DET","PRON","VERB"],"Abbr":["ADJ","ADP","ADV","AUX","CCONJ","DET","INTJ","NOUN","NUM","PART","PRON","PROPN","SCONJ","VERB","X"],"Evident":["ADV","AUX","PART","VERB"],"Polarity":["ADJ","ADV","AUX","CCONJ","INTJ","NOUN","PART","PRON","VERB"],"Person":["AUX","DET","NOUN","PRON","VERB"],"Degree":["ADJ","ADV","DET","NOUN","VERB"],"Polite":["AUX","DET","NOUN","PRON","VERB"],"Clusivity":["ADP","AUX","NOUN","PRON","VERB"]};
function featOnUpos(feat,upos){ const on=FEAT_UPOS[feat]; return !on||!upos||on.indexOf(upos)>=0; }
/* The same question one level down, for the dot-suffixed POS subtypes: which VALUES of a subtype
   feature that word class actually takes. This is what stopped a NOUN's flyout offering NOUN.Fin and
   NOUN.Inf — the FEATURE was scoped per class, the VALUES never were, so they came off a document-wide
   (or UD-wide) list. Trimmed to the classes UPOS_SUBTYPE_ON gates on, so nothing here is dead. */
const SUBTYPE_VALS={"PronType":{"PRON":["Prs","Rcp","Art","Int","Rel","Exc","Dem","Emp","Tot","Neg","Ind"],"DET":["Prs","Art","Int","Rel","Exc","Dem","Emp","Tot","Neg","Ind"],"ADV":["Int","Rel","Exc","Dem","Tot","Neg","Ind"],"ADJ":["Int"]},"NumType":{"NUM":["Card","Ord","Mult","Frac","Sets","Dist","Range"],"DET":["Card","Sets"],"ADJ":["Ord","Mult","Frac","Sets","Dist"],"ADV":["Card","Ord","Mult","Dist"]},"VerbForm":{"VERB":["Fin","Inf","Sup","Part","Conv","Ger","Gdv","Vnoun"],"AUX":["Fin","Inf","Sup","Part","Conv","Ger","Vnoun"],"ADJ":["Part"],"NOUN":["Part","Vnoun"]}};
...
/* ⚠ `upos` NARROWS THE FEATS INVENTORY TO THE FEATURES THAT WORD CLASS TAKES, on request ("in the
   grid, the FEATS dropdown should only show POS-relevant features"). It is the same FEAT_UPOS table
   the AVM's "Other feature…" flyout uses (js/editing/context-menu.js), so the two pickers for the one
   column cannot answer differently — this cell and that menu edit the same FEATS string.
   Two things it deliberately does NOT narrow. A DOC-ONLY key (a feature neither UD nor this table
   knows) is always kept: it is already in the document, so it is evidence, and hiding it would make
   an existing annotation uncompletable. And an UNTAGGED token narrows by nothing at all — there is no
   class to scope by, and the whole inventory is the honest answer, exactly as addFeatureItems treats
   the same case. MISC keys are untouched: they are not features of a word class. */
// The doc-only half of that list, scoped the same way — the analogue of docPairValsForUpos
// (js/editing/context-menu.js) one level up. Scanning the document for keys used on ANY token is the
// very fault this narrowing is for: it put Tense and PronType back on a NOUN, and ten features back
// on a PUNCT, after the table had just taken them off. A key used on THIS class is evidence and stays,
// whether or not UD knows it — that is what keeps a non-UD key already in the document completable.
function docPairKeysForUpos(col,upos){ const set=new Set();
  try{ DOC.forEach(s=>s.tokens.forEach(t=>{ if(t.upos!==upos) return; const raw=t[col]; if(!raw||raw==="_")return;
    raw.split("|").forEach(seg=>{ const eq=seg.indexOf("="); if(eq>0)set.add(seg.slice(0,eq)); }); })); }catch(_){}
  return set; }
function acKeyItems(col,upos){ const base=col==="feats"?Object.keys(UD_FEATS):UD_MISC_KEYS.slice();
  const scope=col==="feats"&&upos&&typeof featOnUpos==="function";
  const out=scope?base.filter(k=>featOnUpos(k,upos)):base.slice();
  (scope?docPairKeysForUpos(col,upos):docPairKeys(col)).forEach(k=>{ if(!out.includes(k))out.push(k); }); return out; }   // UD inventory first, then any doc-only keys
...
function buildFeatEditor(td,sent,t,si,i,key){
  ...
  const editPill=(pill,pt)=>{ arm(); const cur=pill.dataset.val;   // turn the pill back into editable Key=Value text
    ...
    const openIeAC=force=>{ if(document.activeElement!==ie){ if(_acInput===ie)acCloseSoon(); return; }
      ...
      if(eq<0){ if(!seg&&!force){ if(_acInput===ie)acCloseSoon(); return; } kind="key"; q=seg; items=acKeyItems(key,key==="feats"?t.upos:""); }   // before "=" → attribute names (FEATS/MISC inventory); item 9: force-open browses the whole set on an empty segment
  ...
  const featPillMenu=(pill,e)=>{ if(key!=="feats") return false;
    ...
    const items=acKeyItems("feats",t.upos).filter(k=>k===feat||!used.has(k));   // exclude a feature already present elsewhere on this token (a second Case chip serialises fine but is never something the reader wants) — except this pill's OWN key, which must stay selectable so its row can carry the tick
  ...
  const openPillAC=force=>{ if(document.activeElement!==box||box.querySelector("input.pilledit")){ if(_acInput===box)acCloseSoon(); return; }   // not the free-typing caret (e.g. a live chip-edit input) → nothing to complete
    ...
    if(eq<0){ if(!seg&&!force){ if(_acInput===box)acCloseSoon(); return; } kind="key"; q=seg; items=acKeyItems(key,key==="feats"?t.upos:""); }   // before "=" → attribute names; stay silent on an empty segment UNLESS force-opened (item 9: Ctrl+Space browses the whole feature inventory from an empty cell / a caret just outside a pill)
