...
async function fillTranslit(){ if(!hasBridge()||!DOCLANG) return;   // transliteration is enabled only when a model sets the language
  ...
  if(need.size){
    const forms=[...need]; let r;
    try{ r=await window.pywebview.api.transliterate(forms,DOCLANG,TRANSLIT_SCHEME); }catch(e){ if(any)preserveScroll(renderDoc); return; }   // TRANSLIT_SCHEME = the scheme chosen in the status-bar menu ("" ⇒ the language's default)
  ...
  if(any) preserveScroll(renderDoc); }
