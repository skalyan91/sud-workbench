...
function stemma(si,{proj,catNodes}){
  ...
  const LV=48,TOP=20.5,A=16,B=7,realMaxD=Math.max(0,...depth),ny=d=>TOP+d*LV;   // shorter edges; A above clears ascenders; TOP stays small so the root (no incoming edge) gets no extra headroom — raised from 18 by exactly 2.5, on correction, to lower every diagram token uniformly and open genuine headroom above the root
