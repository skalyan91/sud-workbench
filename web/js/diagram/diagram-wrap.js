...
function tree(si){
  ...
  for(let i=0;i<n;i++){const g=E("g",{class:"node"+(sel.s===si&&sel.t===OID(i)?" sel":""),"data-s":si,"data-tok":OID(i)});
    ...
    g.appendChild(hit);
    const dropY=ny(depth[i]);   // seeds the gap from the glyph (node-lbl) row down to whichever row is drawn first below it — STACK_DROP, which used to bump this seed, is removed (diagram-core.js, belowReserveH's own note)
