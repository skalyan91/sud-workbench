...
function bracketsWrapped(si){
  ...
  box._ties=tieLayout(D).rows.map(r=>({kind:r.kind, tier:r.tier, dy:r.dy, pos:r.pos||"", ids:r.ids||null,   // ids: a goeswith row's parts (its own mark is drawn inside the token cell — see wordSpan — so this row only reserves its depth here)
    ...
    fromTok:OID(r.from-1), toTok:OID(r.to-1), avmH:avmRowMaxH(t.slice(r.from-1,r.to))}));   // (ortho → the SCRIPT / sandhi-fused surface form). avmH: the tallest AVM among ONLY this tie's own component tokens (r.from..r.to, 1-based) — not the row-wide max — so htmlTieBottom (diagram-core.js) can fold a component's AVM into the room IT reserves, matching what positionBracketAnnots' own undBot (js/core/document.js) already seats the tie's DRAW at; report: "space below an MWT tie … smaller [in brackets] than other views" / "MWT forms crashing into the next line" once the box's own bottom padding — htmlTieBottom's other caller — was under-reserving too
