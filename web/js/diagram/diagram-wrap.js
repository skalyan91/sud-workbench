...
function bracketsWrapped(si){
  ...
  for(let p=0;p<n;p++){ if(p===root)continue;
    if(!interrupt.has(p)){ const h=head[p]-1; dparent[p]=(h>=0&&h<n&&h!==p)?h:root; continue; }   /* the ordinary case: nest under the real head, same as the flat view.
      ⚠ …AND A TOKEN WITH NO HEAD AT ALL NESTS AT THE TOP LEVEL, which in this structure means under `root`.
      `head[p]` is parseInt'd, so an UNATTACHED token (HEAD `_` — what a partially annotated file writes, and
      now what Clear head leaves behind) gives NaN and `dparent[p]` used to become NaN: `dchildren[NaN]` is
      undefined, and the very next line threw "Cannot read properties of undefined (reading 'push')" — a
      top-level throw that blanks the whole app, on any wrapped bracket view of a sentence with one unattached
      token. Latent long before Clear head existed (any half-annotated CoNLL-U file would do it); reachable from
      the UI now, hence the guard. structure() already takes the same view of such a token — see its own
      root/isNaN branch — so this only makes the bracket nesting agree with it. */
  ...
  const wordW=i=>{ let w=fmeas(t[i],WORD_F); if(relBelow(i))w=Math.max(w,meas(t[i].deprel,POS_F)); if(show.pos)w=Math.max(w,meas(posRowTxt(t[i]),POS_F)); if(hasTr(t))w=Math.max(w,meas(trRowTxt(t[i]),trFont(t[i])));   /* item 28: gated on the ROW being shown, and measured off what it paints — its value or the TIER_EMPTY placeholder */ w=Math.max(w,glossSlotW(t[i]),avmSlotW(t[i])); return w+8; };   // fold in the gloss-tier row width (glossSlotW → 0 when no gloss tier is shown) so a wide morphemic gloss can't crowd its neighbour — the SAME inclusion the stemma/arc layouts already make (item: gloss crowding, wrapped brackets only). item 22: +AVM, same reasoning   // the token cell's width = its widest row (host form only; its folded-punctuation satellites are separate inline spans AFTER the cell)
  ...
  /* ⚠ …PLUS STACKED_GAP, WHICH THE RESERVE ABOVE ALREADY CARRIED AND THIS OFFSET DID NOT. `belowReserveH`
     ...
     bracket's below-stack is not seated by that padding, it is seated by THIS offset off the form's own box
     bottom, and the offset was never told. So the room was reserved and then not used: the whole stack sat
     STACKED_GAP too high and the transliteration row ran straight through the glyph's subjoined marks —
     measured, first Grantha token, translit ink top minus form ink bottom: −6.37px, i.e. a real overlap
     (Javanese −7.86). Reported as "in Sanskrit brackets and outlines, stacked scripts should have the
     appropriate amount of space below tokens (like in arcs)"; arcs takes exactly this quantity at its
     belowStack() seed, which is what "like in arcs" names. With it: +4.88px, against arcs' own +2.80px for
     the same token. 0 for every non-stacking script, magnified or not — nothing else moves. */
  box.style.setProperty("--und-extra",(TOK_TR_GAP+STACKED_GAP)+"px");   // …and the matching shift of .bwund's own top offset, which is measured from the FORM's bottom edge rather than from that padding (app.css)
  ...
  box._ties=tieLayout(D).rows.map(r=>({kind:r.kind, tier:r.tier, dy:r.dy, pos:r.pos||"", ids:r.ids||null,   // ids: a goeswith row's parts (its own mark is drawn inside the token cell — see wordSpan — so this row only reserves its depth here)
    ...
    fromTok:OID(r.from-1), toTok:OID(r.to-1)}));   // (ortho → the SCRIPT / sandhi-fused surface form). item 25/7 round 2: no `avmH` field any more — htmlTieBottom (diagram-core.js) no longer reads one; see that function's own note for why folding AVM in here double-counted against what --undpad/undBot already reserve
  ...
  const wordSpan=i=>{ const grp=document.createElement("span"); grp.className="bwtok"+(sel.s===si&&sel.t===OID(i)?" sel":""); grp.dataset.s=si; grp.dataset.tok=OID(i); grp.style.cursor="pointer"; grp.style.width=wordW(i)+"px"; grp.addEventListener("click",()=>pick(si,OID(i)));   // reserve the widest-row width (rel/POS/translit are absolute, so JS sizes the box as the flex column did before) — a firm `width` (not `minWidth`): wordW already reserves the BOLD width too, so the box never needs to grow when a token is selected; it bolds in place, centred, exactly like unwrapped brackets
    ...
    const below=[];
    if(hasTr(t)){ const rt=trTxt(t[i]); const tr=document.createElement("span"); tr.className="otrans"+(rt&&trRowEdit()?" tr-edit":"")+tierEmptyCls(rt)+frnUp(t[i]); tr.textContent=trRowTxt(t[i]); tr.style.marginTop=descent(POS_F)+"px"; htmlSeamMark(tr,t[i],"translit"); below.push(tr); }   /* item 28: the SVG twin of this row (belowStack, js/diagram/diagram-core.js) and its own reserve (--undpad, hasTr) both give the row to every token in a sentence that romanises anything, so one that romanises nothing paints TIER_EMPTY into it rather than shortening its stack by a row. Not .tr-edit on a placeholder — there is no romanisation under it to open. */   // Item 8: the translit row gains the same descender-matched top gap (+descent(POS_F)) the POS row carries, matching belowStack's per-row step (the .bwund flex column steps 18px per row; this margin folds in the label-font descender)
    belowTiers().forEach(tier=>{ const txt=tierDisp(t[i],tier); const gs=document.createElement("span"); gs.className="gloss gl-edit"+(txt?"":" gl-empty")+frnUp(t[i]); gs.dataset.tier=tier; gs.tabIndex=0; setGlossText(gs,tier,txt||TIER_EMPTY); gs.style.marginTop=descent(POS_F)+"px"; if(tier==="mseg"||tier==="mgloss")htmlSeamMark(gs,t[i],tier); below.push(gs); });   // tierDisp, not tierText: what the row PAINTS (under Latin's macron scheme the MSeg row shows the macronised segmentation while MISC keeps the bare one — see tierDisp, js/core/prefs.js)   // Item 8: each gloss / morphemic-gloss tier gains the same +descent(POS_F) top gap as the POS row; between translit and POS. The segmentation AND morphemic-gloss rows both take the seam mark as an out-of-flow child (both per-morpheme, unlike the lexical gloss row), so .bwund's flex column still centres each row on its own real text — drawn even where THIS tier is the placeholder (TIER_EMPTY; "…" when this was written — not gated on txt), so a seam MSeg marks cleanly never silently vanishes from MGloss just because that one morpheme isn't glossed yet (see the note beside the SVG twin of this call in diagram-core.js)
    if(show.pos){ const p=document.createElement("span"); p.className="bwpos"+tierEmptyCls(t[i].upos); p.textContent=posRowTxt(t[i]); if(t[i].upos)p.title=posTitle(t[i].upos); p.style.marginTop=descent(POS_F)+"px"; below.push(p); }   /* item 28: show.pos alone — the same gate --undpad's own belowReserveH(…,show.pos,…) reserves the row on, so an untagged token fills it with TIER_EMPTY instead of leaving its stack a row short of its neighbours' */   // Item 1: descent(POS_F) extra top gap above the POS row (the .bwund flex column steps 18px per row; this margin folds in the label-font descender so wrapped brackets match belowStack's new belowGap() POS step)
...
// Fan the endpoints of arcs meeting at a shared node: LONGER spans sit further out than shorter ones
// wherever two or more share a side. THE one routine every arc/bracket view calls. Each arc a carries
// head-key hk, dep-key dk, head-x xh, dep-x xd and length len (any monotone-in-width measure); it is
// mutated with offH / offD, the horizontal endpoint offsets.
//
// ⚠️ CORRECTS 9544727's OWN OFFSET MAGNITUDE — that commit is not being re-superseded, its ARITHMETIC is
// being fixed. 9544727 (superseding 29c3130/e8bf96f/4147e31's shared centring target) anchored a side's
// innermost entry (j=0) at `side*w/2` — the width of THE NODE THIS BUCKET BELONGS TO, i.e. its own rendered
// half-width/edge — on the report "an incoming arc lands on the left if it comes in from the left, and on
// the right if it comes in from the right". On correction: "No, that's not what I meant!! Put it back! All
// I meant is that the incoming arc should land half a fanning gap to the left of centre if it comes in from
// the left, and half a fanning gap to the right of centre if it comes in from the right." The DIRECTION-based
// side assignment (Math.sign(xd-xh)/Math.sign(xh-xd) below) was never the problem and stays exactly as
// 9544727 left it; what was wrong was reading "lands on the left" as "AT the node's own edge" rather than
// "a small, fixed distance left of centre". THE ANCHOR IS NOW `spread/2` — half of the SAME uniform fan step
// (fanStep()) every later entry in a bucket already fans by — not a node-size-dependent quantity. A side's
// innermost entry (j=0, the longest span) lands at `side*spread/2`; every later entry in the same bucket
// still fans a further `spread` beyond that anchor (j=1,2,… — e8bf96f/4147e31's own still-correct insight,
// that two arcs sharing a bucket must never land on the exact same pixel, is unchanged; only the point the
// whole bucket fans FROM moved off centre, and by much less than 9544727 had it move).
//
...
function arcsWrapped(si){
  ...
  const placeRows=extraGap=>{ let yCur=0, prevBot=0;
    rows.forEach(r=>{ if(r.ord>0) yCur+=extraGap[r.ord]||0;   // move this line (and every line below it) down by the reserved cross-arc room
      ...
      r.stackBot=r.wordY+belowReserveH(hasTr(t),belowTierN(),show.pos,avmRowMaxH(r.idx.map(i=>t[i])));   // reserve the transliteration + gloss + POS rows below the word (Item 1/8: every below-row folds in descent(POS_F), matching belowStack's descender-matched step). item 22: +AVM, the tallest in THIS ROW — a wrapped view's rows already vary height independently, so this can be tighter than the sentence-wide max the unwrapped views use
...
function treeGeomW(t,n,withLabels){
  ...
  const LV=48,TOP=18,B=7,SPW=meas(" ",WORD_F),NGAP=SPW+4, lw=i=>Math.max(fmeas(t[i],NODE_F),glossSlotW(t[i]),avmSlotW(t[i])), hgw=i=>tailW(t[i],NODE_F), ldw=i=>leadW(t[i],NODE_F), elw=i=>(withLabels&&i!==root)?meas(t[i].deprel,POS_F):0;   // item 13: node slot also fits the gloss tiers. item 22: +AVM, same reasoning. TOP matches tree()'s own (below) — raising it to "lower the tokens" was tried and reverted there, see stemma()'s note (js/diagram/diagram-render.js); TOK_Y_LOWER (applied per token, at the draw site, never to this geometry) is the fix that actually works
...
function projWrapped(si,kind){
  ...
  const layout=depW=>{   // greedy wrap by each UNIT's slot width; an MWT never splits and its (possibly wider) fused form is reserved so it can't overflow the inline-end
    ...
    const inkHalf=idx=>{ const tk=t[idx]; let h=fmeas(tk,WORD_F)/2+1.9;   // form ink half + 3.5px paint-order halo
      if(show.pos){ const pd=posRowTxt(tk); h=Math.max(h, meas(pd,POS_F)/2 + pd.length*LSpx + 1.7); }   // item 28: the placeholder is a painted row like any other, so it is measured like one   // POS ink half + full letter-spacing run + 3px halo
  ...
  const PADV=8, belowH=belowReserveH(hasTr(t),belowTierN(),show.pos,avmRowMaxH(t));   // Item 1/8: every below-row (translit, each gloss, POS) folds in descent(POS_F), matching belowStack's descender-matched step. item 22: +AVM, the tallest in the sentence — this function's ONE UNIFORM row height (see the note above) needs the sentence-wide max, not a per-row one
...
function wpDraw(box){ const wp=box._wp; if(!wp) return;
  ...
  wp.edges.forEach(e=>{ const g=E("g",{class:"edge-g"+(sel.s===wp.si&&sel.t===wp.oid[e.d]?" sel":""),"data-s":wp.si,"data-dep":wp.oid[e.d],"data-head":wp.oid[e.h]});
    g.appendChild(E("path",{class:"edge-hit",d:e._d}));   // item 29: the same fat invisible hit stroke the stemma's own edges take — see diagram-render.js for why a 1.4px line is not a right-click target
...
function tree(si){
  ...
  /* ⚠ THE RESERVE BELOW THE STACK IS TR_TIGHTEN SHORTER ("the gap below a transliteration should be
     ...
     ⚠️ AND aboveExtra IS THE MIRROR OF IT, asked for in the same breath two rounds later ("add back the 2px
     of space below the transliteration, but add an extra 4px of space above the token"). It goes to nyD —
     the space above a token is opened by dropping the glyph, the incoming edge ending at ny(depth)−A
     whatever the glyph does — and then ONCE to belowReserve, purely to CARRY that drop so the stack below
     does not overrun its own reserve. Once, not twice: trDrop above is added twice because its own sentence
     asked for the gap below to widen as well, and this one asks only about the space ABOVE.
     ⚠️ THE trLayer() GATE ON IT IS LOAD-BEARING, not house style. The rest of the same request is "if there
     is no transliteration, make the token spacing exactly the same as in stemmas", and with the gate every
     term this notation adds beyond the stemma's own (belowTighten, trDrop, belowExtra, aboveExtra) is 0
     there, so nyD reduces to nyL+TOK_Y_LOWER+NODE_Y_EXTRA — the stemma's expression exactly, LV back to a
     bare 48, and the two notations measured identical (token baseline, level step, edge clearances) with the
     transliteration row off. The third case that sentence names — "if there is only transliteration, use the
     same spacing as for English" — needs nothing here and is satisfied at source: only-transliteration means
  ...
  genericEntries.forEach(ge=>{ const i=ge.i, col=relColor("subj"), ink=arcInk(col);
    ...
    const glbl=E("text",{class:"node-lbl",x:ge.gx,y:ge.gy+TOK_Y_LOWER+NODE_Y_EXTRA+trDrop+aboveExtra}); glbl.textContent="∅"; g.appendChild(glbl);   // a virtual NODE, so it lowers with every real one — trDrop and aboveExtra included (see belowReserve's own note above); its edge above and its box below both stay on the layout level
  ...
  for(let i=0;i<n;i++){const g=E("g",{class:"node"+(sel.s===si&&sel.t===OID(i)?" sel":""),"data-s":si,"data-tok":OID(i)});
    ...
    /* ⚠ …LESS TR_TIGHTEN (js/diagram/diagram-core.js), the hierarchy's own `0.5em − 0.5ex` reduction of the
       ...
       ⚠️ `belowTighten`, NOT `TR_TIGHTEN` ITSELF — the trLayer()-gated copy of it that `belowReserve` above
       already uses. Written bare here, this reduction fired even with the transliteration row OFF, where
       there is no token→transliteration step to tighten at all: the drawn stack (a gloss tier, say) then sat
    ...
    if(hasTr(t)){ const rt=trTxt(t[i]), rd=trRowTxt(t[i]); { const ty=dropYD+belowGap(); const e=E("text",{class:"translit"+tierEmptyCls(rt)+frnUp(t[i]),x:x[i],y:ty,"text-anchor":"middle"}); e.textContent=rd; if(rt&&trRowEdit())e.classList.add("tr-edit"); g.appendChild(e); boxes.push({x:x[i],y:nyD+14,hx:meas(rd,trFont(t[i]))/2,hy:7}); }   /* no seam mark: this row hangs under a NODE, not under a word in reading order — see the node's own call below */ }   // Item 8: same descender-matched top gap (belowGap()) the other renderers give the translit row   // …and nyD+14, never nyL+14 — see the note just above
    belowTiers().forEach((tier,ti)=>{ const step=(hasTr(t)?belowGap():0)+(ti+1)*(belowGap()), gyD=dropYD+step, txt=tierDisp(t[i],tier), dtxt=txt||TIER_EMPTY;   /* item 28: hasTr(t), not trTxt(t[i]) — the translit row is now drawn (placeholder and all) for every node whenever the sentence romanises anything, so the tiers under it step down by the same amount at every node. Keyed on the token alone, a node that happened to have no romanisation used to pull its glosses up into the translit row's own line while its neighbours' stayed below it. */ const e=E("text",{class:"gloss gl-edit"+(txt?"":" gl-empty")+frnUp(t[i]),x:x[i],y:gyD,"text-anchor":"middle","data-tier":tier,tabindex:"0"}); setGlossText(e,tier,dtxt); g.appendChild(e); boxes.push({x:x[i],y:gyD-4,hx:meas(dtxt,trFont(t[i]))/2,hy:7});   // gyD−4, not the old layout-level gy−4: the box states where this row is DRAWN, for the reason the note above the translit row gives
      });   /* …and no seam mark on the morphemic rows either, for the same reason as the transliteration row just above: they sit under a depth-placed NODE. (Wrapped brackets' identical pair of rows, which DO sit in reading order, keep theirs — see bracketsWrapped.) */   // gloss / morphemic tiers under the node (hierarchy has no per-node POS row) — the segmentation AND morphemic-gloss rows carry the seam mark (both per-morpheme, drawn off the placeholder's own width (TIER_EMPTY; "…" when this was written) where this tier isn't annotated for this token — see the note beside diagram-core.js's belowStack); the lexical gloss row doesn't
    ...
    { const STEP=belowGap(), nodeBot=dropYD+(hasTr(t)?STEP:0)+belowTierN()*STEP;   // item 28: hasTr(t) — the same sentence-wide translit row the tiers above now step past at every node
      if(show.avm) drawAVM(g,x[i],nodeBot+avmTopGap(),t[i],null,null,loB); }   // item 28: gated on the TIER — drawAVM paints TIER_EMPTY for a node whose token has no matrix   // item 25/4 round 2: avmTopGap() — see belowStack's own identical change/note; not row-specific (it converts one MORE belowGap() step into a box-top offset using the box's own metrics, whatever row precedes it)
...
function brackets(si){
  ...
  const wx=Array(n),wlo=Array(n),whi=Array(n),wform=Array(n),genericX=Array(n).fill(null); let x=2; seq.forEach(it=>{ let ww, ld=0;
    if(it.t==="w"){   // always size the slot to the UNBOLD form → a selected word bolds in place (centred, symmetric), never reflowing its neighbours
      ...
      ww=Math.max(fw, show.pos?meas(posRowTxt(t[it.i]),POS_F):0, trLayer()?meas(trRowTxt(t[it.i]),trFont(t[it.i])):0,   /* item 28: what each row paints — value or placeholder */ glossSlotW(t[it.i]), avmSlotW(t[it.i])); const r=relOf(it.i); if(r) ww=Math.max(ww,meas(r,POS_F));   // item 3: fold in the gloss/MSeg/MGloss row width (glossSlotW → 0 when no gloss tier is shown) so a long gloss under a SHORT form — typically an MWT component (sat/ādi) with a long MSeg/MGloss — can't crowd its neighbour. The stemma/arc/wrapped-bracket layouts already reserve this; unwrapped brackets was the last one missing it. item 22: +AVM, same reasoning
  ...
  // further out than shorter ones, one fanStep() apart
  ...
  seq.forEach(it=>{ if(it.t==="w"){
      ...
      const brkLift=BRK_LIFT;   // hoisted out of the per-bracket loop — see its own note above the seq walk
...
function outline(si){const D=displaySent(DOC[si]), t=D.tokens, n=t.length, OID=k=>D.map[k]+1, sent={tokens:t},{children,root}=structure(sent); RTL=D.rtl;
  ...
  /* ⚠ …AND A STACKING SCRIPT'S ROW GETS STACKED_GAP OF REAL ROOM UNDER IT, the same quantity every SVG
     notation's belowStack() seeds and wrapped brackets' own `--und-extra` now carries ("in Sanskrit brackets
     and outlines, stacked scripts should have the appropriate amount of space below tokens (like in arcs)").
     The outline is the ONE view with no below-stack at all — its tiers run inline along the row — so "space
     below the token" here is literally the row's bottom padding, and `.oline{line-height:1}` sizes the row
     off the font-size while a Grantha or Javanese akshara's subjoined consonants reach far past it. Measured,
     first row of the Grantha sample: the form's ink ended 8.02px BELOW the row box and left 2.09px to the next
     row's ink (English 17.20, lzh 14.04, Devanagari 6.89). With it: 3.23px inside the box, 13.34px to the next
     row. 0 for every non-stacking script — STACKED_GAP is 0 there — so no other document's rows move. */
  const _half=descent(NODE_F)/2, _magHalf=MAG_DESC/2; box.style.setProperty("--olpt",(5-_half+_magHalf)+"px"); box.style.setProperty("--olpb",(5+_half-_magHalf+STACKED_GAP)+"px");
  ...
  /* ⚠ THE WALK IS FROM THE ROOT, SO ANYTHING THE ROOT CANNOT REACH WOULD SIMPLY NOT BE LISTED — and this is the
     one notation where that means the token DISAPPEARS rather than merely losing its edge (every other view
     draws tokens in reading order and just omits the arc). An UNATTACHED token (HEAD `_` — a half-annotated
     file, or one the reader has just detached with Clear head) is exactly that case, and so is the far side of a
     head CYCLE: neither is in any `children` list the root's descent reaches. `listed` makes the walk
     idempotent, and the sweep after it lists whatever the descent missed at top level — the SAME fallback
     structure() itself applies when it assigns a depth to every token that its own root-first pass never
     visited. Declared as a function rather than the IIFE this used to be, so that sweep can call it. */
  const listed=new Set();
  function w(i,d,anc,reps){ if(listed.has(i))return; listed.add(i); const chain=anc.concat(OID(i));   // ancestors-or-self, so a selection can light up the whole subtree
    ...
    if(hasTr(t)){ const rt=trTxt(t[i]); const tr=document.createElement("span"); tr.className="otrans"+(rt&&trRowEdit()?" tr-edit":"")+tierEmptyCls(rt)+frnUp(t[i]); tr.textContent=trRowTxt(t[i]); row.appendChild(tr); }   // right next to the form   // item 28: the tier is showing for this sentence (hasTr) → a token with nothing to romanise says so with TIER_EMPTY rather than silently dropping out of the row
    ...
    ghostsAt[i].forEach(({gi,rel,kind})=>{ const grow=document.createElement("div"); grow.className="oline oline-ghost"+(inSel(gi)?" sel":"");   // item 3: highlighted like a real row when ITS token is the current selection
      ...
      const gpos=document.createElement("span"); gpos.className="opos"+tierEmptyCls(t[gi].upos); gpos.textContent=posRowTxt(t[gi]); if(t[gi].upos)gpos.title=posTitle(t[gi].upos); grow.appendChild(gpos);   // item 28: a ghost row shows its own token's POS, placeholder included, like the real row it duplicates
  ...
  w(root,0,[],[]);
  for(let i=0;i<n;i++) if(!listed.has(i)) w(i,0,[],[]);   // …and everything the root's own descent could not reach, at top level, in reading order
  return box;}
