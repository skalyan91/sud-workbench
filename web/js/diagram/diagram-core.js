...
function smpReshape(root){
  ...
  for(const el of texts){ const s=ownText(el);
    ...
    /* ⚠ ROUND FIFTEEN — "Lower by 0.25px." Continues on the same script/axis as round fourteen
       (Siddham's vertical): 3.25 -> 3.0. */
    const ARC_SEAM_LIFT_PX={Siddham:3.0, ZanabazarSquare:2.5, Grantha:-2, Kawi:2};
...
// callsite that hasn't been taught to pass one yet, or for a sentence/row with AVM off or no FEATS at all) —
...
/* RE-INVESTIGATED AGAIN, DEEPER THAN c6bca96, ON REPORT ("its AVM padding is not even like it is in LTR")
   ...
   // AVM_BRK_W=5 — verified against real rendered path `d` attributes, both literally "5"). The report is
   // about how a tick READS, not a raw px mismatch: the AVM box sits at 10.5px text against the diagram's own
   // 15px reference (WORD_F/POS_F/MWT_F) — a 5px tick that reads as a modest hook on a 15px-scale bracket
   // spanning a whole multi-character MWT reads as a much CHUNKIER fraction of a 10.5px-scale AVM box, whose
   // rows and padding were JUST tightened further still (round 3/4 above). 3.5 = 5 × (10.5/15), the same
   // scale ratio the rest of this file already leans on wherever a Latin-annotation element sized off the
   // 10.5px AVM tier needs to read as equivalent to something drawn at the 15px token reference (weightCurve()
   // is the identical idea one axis over — font WEIGHT compensating for a smaller SIZE reading lighter; this
   // is bracket-serif LENGTH compensating for a smaller SIZE reading chunkier). AVM_BRK_EXT is UNTOUCHED — it
   // is not a font-scale quantity at all: it is still the same literal mwtTie's own end-pins use, half of
   ...
   // side's own larger margin, which was what was asked for AT THE TIME (round 3's own report was for "a
   // bit more room", explicitly not full parity) — see round 4, directly below, for the follow-up that
   // supersedes this.
   // AVM_PAD_L round 4 (this session, immediate follow-up to round 3): "Wait a minute — if the right gap
   // is 6.10px, then I want the left gap to be the same!" — an explicit numeric parity ask, not another
   // estimate. Re-measured BOTH gaps live and fresh rather than trusting round 3's own 5.74/6.10 figures
   // as still current (the task that produced this round said so explicitly, and it was the right call:
   // re-measuring found the RIGHT gap itself hadn't drifted, but also that "the left gap" and "the right
   // gap" are not single box-wide numbers — text-anchor="end" on .avm-attr and text-anchor="start" on
   // .avm-val mean each ROW's own gap depends on how far that row's own attr/val STRING falls short of
   // the box's shared column-width reserve (L.attrW/valW, the max over every row in the box), not only on
   // AVM_PAD_L/AVM_PAD_R. A narrow row (e.g. this box's "TAM" attr label beside "VOICE") reads a much
   // bigger left gap than a row whose own string fills the reserve. Measured the specific row the report
   // named implicitly by naming the token — samples/english.conllu, "conceived", Voice=Pass, the row whose
   // own strings are closest to their column's reserve in this box, hence the most representative single
   // number for "the" gap on each side: at then-current AVM_PAD_L=5.75, leftGap=2.25px, rightGap=2.60px
   // (getBoundingClientRect() on the live .avm-attr/.avm-val <text> elements and the live
   // .mwt-tie-cas/.mwt-tie-h bracket <path> elements, exactly avmBrkInkDx()'s own established technique —
   // NOT getBBox(), which 1f104a8 already proved blind to stroke geometry). Solved algebraically first,
   // then confirmed live rather than trusting the algebra alone (this thread's own standing rule, after
   // 1f104a8/2e73d4f found padding arithmetic and rendered ink don't always move in perfect lockstep):
   // with the box centred on a fixed cx (x0=cx-w/2, x1=cx+w/2, drawAVM below) and w itself growing by
   // whatever AVM_PAD_L grows by, a change of δ in AVM_PAD_L shifts x0 left by δ/2 and attrX right by
   // δ/2 more than that (attrX=x0+AVM_PAD_L+dx+attrW), a net +δ on the LEFT gap — while x1 and valX both
   // shift by the same +δ/2 as each other, cancelling to a net 0 on the RIGHT gap. So AVM_PAD_L alone can
   // reach any target left gap without ever moving the right one, confirming the task's own instruction
   // that only AVM_PAD_L was in scope held up structurally, not just by assumption. Target
   // AVM_PAD_L=5.75+(2.60−2.25)≈6.10; set it, then RE-measured live (not just trusted the arithmetic):
   // leftGap=2.591px, rightGap=2.587px on the same Voice=Pass row, Δ0.004px — inside the ~0.05px tolerance
   // this thread has used throughout, and confirming the 1:1 relationship held exactly this round (it has
   // not always, elsewhere in this saga — see round 3's own AVM_BRK_EXT investigation for a case where a
   // constant's effect on rendered ink was NOT what a naive read of the formula would suggest). Other rows
   // in the SAME box (TAM, whose own "TAM"/"Past" strings are narrower than "VOICE"/"Pass") do NOT come
   // out equal (leftGap≈10.5px, rightGap≈3.6px at the new value) — this is not a bug or a miss, it is the
   // same text-anchor-driven per-row spread described above, present before this round and unaffected by
   // it: AVM_PAD_L/AVM_PAD_R are box-wide constants, so "equal gaps" can only ever be a property of the
   // row that defines the column reserve (here, Voice), not a guarantee for every row in every box.
   // AVM_COL_GAP round 5 (new session, direct follow-up report): "the gap between attrs and vals should be
   // no more than the padding within the brackets (ideally equal)" — then, clarifying what "padding within
   // the brackets" meant: "I meant the brackets padding plus the tick length." VERIFIED FIRST, not assumed:
   // re-read drawAVM's own attrX/valX formulas (this file, below) line by line. attrX=x0+AVM_BRK_W+
   // avmBrkInkDx()+(AVM_PAD_L-AVM_BRK_W)+L.attrW simplifies (dx=0 today) to x0+AVM_PAD_L+L.attrW — i.e.
   // AVM_PAD_L is ALREADY, by construction, the full "tick length (AVM_BRK_W) + pure clearance beyond it"
   // span, measured from the box's own nominal outer edge x0 to the attr column's ink — exactly the
   // "padding plus tick length" quantity the report named, confirming AVM_PAD_L (not just AVM_PAD_L−AVM_BRK_W,
   // the pure-clearance piece alone) is the right reference. valX=attrX+AVM_COL_GAP, with NO bracket-width
   // term on either side (there is no physical bracket between the attr and val columns) — so AVM_COL_GAP
   // is already, directly, the FULL quantity comparable to AVM_PAD_L, not something needing a tick term
   // added to it first.
   // The naive expectation (AVM_PAD_L/AVM_PAD_R's own precedent: formula value ≈ live ink gap, glyph-bearing
   // noise <1.5px) did NOT hold here — measured LIVE first, not assumed: pywebview evaluate_js against a
   // real launched app.__main__.main() instance (background probe thread; a fresh path with no prior
   // autosave/snapshot was used — samples/english.conllu's own on-disk copy had accumulated dirty edits
   // from earlier sessions in this saga, so opening it directly restored THAT stale content, not the
   // pristine Gettysburg-address text this comment's own history refers to; copying the file to a path with
   // no snapshot side-stepped it), samples/english.conllu, "conceived" (Tense=Past|VerbForm=Part|Voice=Pass),
   // the SAME Voice=Pass row round 4 above used, for direct comparability. getBoundingClientRect() on the
   // live .avm-attr/.avm-val elements (both HarfBuzz-shaped <path>s for a standalone row like Voice — see
   // item 27's own note) and the live .avm-hit rect (nominal x0+AVM_BRK_W/x1−AVM_BRK_W, unaffected by this
   // change — see its own note below drawAVM). At the OLD AVM_COL_GAP=6: live colGap (val's left ink − attr's
   // right ink) = 8.086px against a live padLeft (attr's left ink − x0, i.e. AVM_PAD_L's own live ink
   // manifestation) of 6.100px — NOT close (Δ1.986px), unlike AVM_PAD_L/R's own well-behaved history. Why:
   // AVM_PAD_L/R's ink gap has only ONE glyph's bearing in play (the bracket's other "edge" is a straight
   // stroke with no font of its own), but AVM_COL_GAP's ink gap has TWO — the attr string's OWN right-side-
   // bearing (its last glyph, small-caps "E" here) AND the val string's OWN left-side-bearing (its first
   // glyph, "P" here) both eat into the same nominal span, compounding rather than cancelling. Confirmed
   // this is real and stable, not a stale-shape read (the async HarfBuzz settle race 35b8572's own note
   // documents): re-measured across all 60 live AVM rows in the doc, waited 4s post-render for shapes to
   // settle, values were bit-identical across repeat measurements, and the SAME attr word at a DIFFERENT
   // val string (e.g. "DEFINITE"/"Ind" → colGap 7.53px vs "DEFINITE"/"Def" → 8.08px) produced a DIFFERENT
   // colGap, confirming the two-glyph-bearing explanation rather than measurement noise.
   // FIX — solved algebraically from the live data, then confirmed live rather than trusting the algebra
   // alone (this thread's standing rule): offset = liveColGap − AVM_COL_GAP = 8.086−6 = 2.086px (the
   // combined bearing, assumed constant per glyph pair, i.e. linear in AVM_COL_GAP — the SAME assumption
   // round 4 made for AVM_PAD_L and confirmed held). Target = AVM_PAD_L itself (6.1, the live-confirmed
   // "padding+tick" quantity — see above). New AVM_COL_GAP = 6.1−2.086 ≈ 4.014. Set it, re-measured live:
   // colGap=6.096px at AVM_COL_GAP=4.01 (Δ0.004px against the 6.100px target — already inside tolerance),
   // one more hair of precision to 4.014 brought it to colGap=6.1002px, Δ0.0002px against padLeft=6.0999px
   // — a smaller residual than round 4's own accepted 0.004px. AVM_COL_GAP dropped from 6 to ~4.01, a
   // real, non-trivial shrink (not the "small, almost negligible" change a naive extrapolation from
   // AVM_PAD_L/R's own history would have predicted) — exactly because AVM_COL_GAP's ink gap carries twice
   // the glyph-bearing distortion AVM_PAD_L/R's own ink gaps do, once discovered by measuring rather than
   // assumed by analogy. w=AVM_PAD_L+AVM_PAD_R+attrW+AVM_COL_GAP+valW+2*avmBrkInkDx() (avmLayout, below)
   // already references AVM_COL_GAP symbolically, so the box's own total-width reserve absorbs the change
   // with no other line needing to move. Like round 4's own AVM_PAD_L/R gaps, this is a box-wide constant
   // measured against ONE representative row (Voice=Pass, the row round 4 itself established as
   // representative) — other rows' own colGap ink varies by their own boundary glyphs (a GROUP row's val
   // stays native SVG text rather than a HarfBuzz path — see item 27's own note — and reads a noticeably
...
// item 23 round 2 — CORRECTED. Item 23 read "AVM brackets should be styled and spaced exactly like MWT
// brackets" as license to draw a bracket per FEATURE PAIR (`.oavm-item::before/::after`, one border-drawn
// "[" "]" around each "ATTR val"), dropping the literal glyphs entirely. That was the wrong grouping: what
// was actually asked for is the traditional interlinear notation's own convention — ONE bracket pair around
// the WHOLE attribute-value MATRIX, not one around every attribute individually (a real AVM, HPSG-style or
// interlinear, is `[ATTR1 val1  ATTR2 val2]`, a single feature structure, never `[ATTR1 val1] [ATTR2 val2]`
// as if they were unrelated). So: literal "[" "]" TEXT NODES are back per item — plain running text, not a
// drawn glyph, exactly avmInline's OWN pre-93fa0d8/pre-item-23 behaviour (see git history) — and the
// border-drawn bracket technique item 23 introduced MOVES from `.oavm-item` to `.oavm` itself (app.css),
