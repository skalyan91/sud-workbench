...
_SCRIPT_SCHEMES: dict[str, list[tuple[str, str]]] = {
    ...
    # the FORM column, the grid and the editors keep what the file says.  See `app/macron.py` for
    # where the lengths come from and why the row can list unavailable.
    ...
    try:
        ...
        if not out:
            if scheme == "iast":
                ...
                # its engine is the only one that reads more than the surface: the lookup is keyed
