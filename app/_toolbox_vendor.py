"""Vendored copy of acoli-repo/toolbox_py `toolbox.py`.

Source: https://github.com/acoli-repo/toolbox_py (commit 27bdaa3), MIT-licensed.
Vendored because the upstream package ships a broken wheel (the module is not
declared in its pyproject, so `pip install` installs only dist-info) and its
source does `from collections import Sequence`, which was removed in Python 3.10.
The sole edit here is that import, redirected to `collections.abc` (see below).
Kept isolated in `app/` so all toolbox-package usage lives behind
`app/toolbox_import.py`, mirroring how the app isolates other optional deps.
"""
import os.path
from os import listdir
import re
from collections import OrderedDict, deque
from collections.abc import Sequence  # Py3.10+: Sequence moved out of `collections`
import logging
import warnings
